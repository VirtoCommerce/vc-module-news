...
const pagination = useDataTablePagination({
  ...
  onPageChange: ({ page }) => {
    ...
    return searchNewsArticlesDrafts();
  },
});
...
const title = computed(() => `${t("VC_NEWS.PAGES.LIST.TITLE")}: ${t("VC_NEWS.MENU.DRAFTS")}`);
...
onMounted(async () => {
  await searchNewsArticlesDrafts();
});
...
exposeToChildren({
  reload: searchNewsArticlesDrafts,
  reOpenDetailsBlade,
});
</script>
